...
from src.cache import get_cache, key_gen
from src.cache.cache_service import AbstractCache
from src.config import settings
from src.database.actions import SubMenuAction, get_submenu_orm
from src.models import SubMenu, schemas
from src.services.base_servises import Service


class SubMenuService(Service):
    def __init__(
        self, cache: AbstractCache, service_orm: SubMenuAction, cache_key: str
    ):
        self.cache = cache
    ...
    async def get_list(
        self, menu_id: UUID, skip: int = 0, limit: int = 10
    ) -> list | dict:
        result: list | dict | None = await self.cache.get_cache(
            key=key_gen(menu_id, self.all_cache_key)
        )
        if not result:
            submenus: list[SubMenu] = await self.service_orm.get_all_with_relates(
                menu_id=menu_id, skip=skip, limit=limit
            )
            result = list(self.service_orm.serialize(submenu) for submenu in submenus)
            await self.cache.set_cache(
                data=result, key=key_gen(menu_id, self.all_cache_key)
            )
        return result
    ...
    async def get(self, menu_id: UUID, submenu_id: UUID) -> dict:
        ...
        if not result:
            submenu: SubMenu = await self.service_orm.get_with_relates(
    ...
    async def update(
        self, menu_id: UUID, submenu_id: UUID, data: schemas.SubMenuUpdate
    ) -> dict:
        if not await self.service_orm.check_exist_submenu(
            menu_id=menu_id, submenu_id=submenu_id
        ):
            raise HTTPException(detail="submenu not found", status_code=404)
...
def get_submenu_service(
    ...
    return SubMenuService(
        cache=cache, service_orm=service_orm, cache_key=settings.App.SUBMENU_CACHE_KEY
    )
