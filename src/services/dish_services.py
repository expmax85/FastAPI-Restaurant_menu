...
from src.database.actions import DishAction, get_dish_orm
from src.models import Dish, schemas
...
class DishService(Service):
    ...
    async def create(
        self, menu_id: UUID, submenu_id: UUID, data: schemas.DishCreate
    ) -> dict:
        ...
        await self.cache.set_cache(
            data=result, key=key_gen(menu_id, submenu_id, result.get("id"))
        )
        await self.cache.delete_cache(
            key=key_gen(menu_id, submenu_id, self.all_cache_key)
        )
        await self.cache.delete_cache(
            key=key_gen(menu_id, settings.App.SUBMENU_CACHE_KEY)
        )
        await self.cache.delete_cache(key=key_gen(menu_id, submenu_id))
        return result

    async def get_list(
        self, menu_id: UUID, submenu_id: UUID, skip: int = 0, limit: int = 10
    ) -> list | dict:
    ...
    async def get(self, menu_id: UUID, submenu_id: UUID, dish_id: UUID) -> dict:
        ...
        if not result:
            dish: Dish = await self.service_orm.get_with_relates(
    ...
    async def update(
        self, menu_id: UUID, submenu_id: UUID, dish_id: UUID, data: schemas.DishUpdate
    ) -> dict:
...
def get_dish_service(
    ...
    return DishService(
        cache=cache, service_orm=service_orm, cache_key=settings.App.DISH_CACHE_KEY
    )
