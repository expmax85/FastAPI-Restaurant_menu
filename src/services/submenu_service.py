...
class SubMenuService:
    ...
    async def create(self, menu_id: UUID, data: schemas.SubMenuCreate) -> SubMenu:
        ...
        await self.cache.set_cache(data=values, key=key_gen(menu_id, submenu.id))
    ...
    async def get_list(self, menu_id: UUID, skip: int = 0, limit: int = 10) -> list[SubMenu]:
    ...
    async def update(self, menu_id: UUID, submenu_id: UUID, data: schemas.SubMenuUpdate) -> SubMenu:
