...
class MenuService(Service):
    ...
    async def get_list(self, skip: int = 0, limit: int = 10) -> list | dict:
        result: list | dict | None = await self.cache.get_cache(
            key=key_gen(self.all_cache_key)
        )
        if not result:
            menus: list[Menu] = await self.service_orm.get_all_with_relates(
...
def get_menu_service(
    ...
    return MenuService(
        cache=cache, service_orm=service_orm, cache_key=settings.App.MENU_CACHE_KEY
    )
