...
async def test_get_submenus(test_app):
    menu = await actions.menu_orm.create({'title': 'Test menu 1', 'description': 'Test description 1'})
    await actions.submenu_orm.create({'title': 'Test submenu 1', 'description': 'Test subdescription 1'},
                                       menu_id=menu.id)
...
async def test_update_menu(test_app):
    ...
    submenu = await actions.submenu_orm.create({'title': 'Test submenu 1', 'description': 'Test subdescription 1'},
                                                 menu_id=menu.id)
    response = await test_app.patch(f"/api/v1/menus/{menu.id}/submenus/{submenu.id}",
